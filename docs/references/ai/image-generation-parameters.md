...
1. Pick (or reuse) the **canonical key**. If new, add one row to `IMAGE_PARAM_CATALOG` (`{ schema, wire? }`) — set `wire` **only** if the vendor name isn't the auto snake_case form. Add its `KEY_LABELS` row, then complete every locale and validation step in the [i18n workflow](../../guides/i18n.md#translation-completion-in-pull-requests).
