...
Options:
  ...
  --repo OWNER/REPO      GitHub repository (default: resolve the origin remote).
...
for command_name in base64 gh git openssl tr; do
...
if [[ -z "$repository" ]]; then
  ...
  if ! origin_url="$(git -C "$repository_root" remote get-url origin 2>/dev/null)"; then
    echo "Could not read this checkout's origin remote. Pass --repo OWNER/REPO." >&2
    exit 1
  fi
  if ! repository="$(
    gh repo view "$origin_url" --json nameWithOwner --jq .nameWithOwner 2>/dev/null
  )"; then
    echo "Could not resolve the origin remote '$origin_url'. Pass --repo OWNER/REPO." >&2
    exit 1
  fi
fi
...
certificate_subject=""
p12_error_path="$(mktemp -t cherry-studio-p12-error)"
